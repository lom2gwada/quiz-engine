...
const MODES: GameMode[] = ['classic', 'timeAttack', 'noMistake']
...
export function LeaderboardPanel({ quizTitle, userId }: { quizTitle: string; userId?: string | null }) {
  ...
  const modeLabel = (m: GameMode) => m === 'timeAttack' ? t('start.mode.timeAttack') : m === 'noMistake' ? t('start.mode.noMistake') : t('start.mode.classic')
