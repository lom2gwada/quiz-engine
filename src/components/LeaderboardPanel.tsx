...
import { fetchLeaderboard, type LeaderboardRow, type RankedMode } from '../utils/leaderboard'
import { formatDuration } from '../utils/time'
import { MODE_ICONS } from './HistoryPage'

const MODES: RankedMode[] = ['classic', 'timeAttack', 'noMistake']
...
export function LeaderboardPanel({ quizTitle, userId }: { quizTitle: string; userId?: string | null }) {
  const t = useT()
  const [mode, setMode] = useState<RankedMode>('classic')
  ...
  const modeLabel = (m: RankedMode) => m === 'timeAttack' ? t('start.mode.timeAttack') : m === 'noMistake' ? t('start.mode.noMistake') : t('start.mode.classic')
  const valueLabel = (row: LeaderboardRow) => {
    if (mode === 'classic') return `${row.value}%`
    if (mode === 'timeAttack') return t(row.value === 1 ? 'leaderboard.correctCount.one' : 'leaderboard.correctCount.other', { n: row.value })
